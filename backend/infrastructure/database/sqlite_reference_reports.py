...
class ReferenceReports:
    ...
    def preview(doc: dict[str, Any], *, already: bool = False) -> dict[str, Any]:
        return {
            ...
            "new_count": 0,
            "source_cell_count": sum(len(s["cells"]) for s in doc["sheets"]),
            "position_count": len(doc.get("recognition", {}).get("structural_actions", [])),
            "new_reference_count": 0,
            "skipped_count": 0,
            "changed_count": 0,
            "same_count": 0,
            "error_count": len(doc.get("recognition", {}).get("issues", [])),
            "issues": doc.get("recognition", {}).get("issues", []),
            "recognition": doc.get("recognition", {}),
