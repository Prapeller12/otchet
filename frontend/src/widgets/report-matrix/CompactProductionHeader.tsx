  ...
  return <section className="compact-production-header" aria-label="Шапка изделия">
    <form onSubmit={event => { event.preventDefault(); void save(); }}>
      <fieldset disabled={blocked || busy} className="compact-production-row">
        ...
        {dirty && <button type="button" className="mini-button compact-header-action" onClick={cancel} aria-label="Отменить изменения шапки" title="Отменить изменения шапки">×</button>}
      </fieldset>
    </form>
