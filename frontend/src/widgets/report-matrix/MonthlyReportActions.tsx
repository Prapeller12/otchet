...
export function MonthlyReportActions({ gateway, query, revision, title, blocked }: {
  gateway: ApplicationGateway; query: ExportRequest; revision: string; title?: string; blocked: boolean;
}) {
  ...
  useEffect(() => {
    ...
    if (!blocked && gateway.getReportVerification) {
      void gateway.getReportVerification({ report_type, organization_id, ...(year ? { year } : {}), month, expected_revision: revision })
    ...
    return () => { active = false; };
  }, [gateway, report_type, organization_id, year, month, revision, title, blocked]);
  if (!gateway.exportPdf || !gateway.getReportVerification || !gateway.verifyReport) return null;

  async function pdf() {
    setBusy(true); setError(""); setMessage("");
    try {
      const result = await gateway.exportPdf!({ ...query, month, expected_revision: revision });
  ...
  async function verify() {
    ...
    try {
      const result = await gateway.verifyReport!({ ...query, month, expected_revision: revision, signer_name: signer.trim(), confirmed, snapshot_sha256: verification.snapshot_sha256 });
