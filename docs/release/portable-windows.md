...
`test-v0.1.0-dev.33`). В архив входят launcher, Python runtime, frontend, миграции,
ресурсы и инструкция `TESTING.txt`.

Тестовая сборка включает проверенный WebView2 Fixed Runtime. Workflow проверяет
упакованный self-test, настоящее окно после распаковки в путь с кириллицей и повторное
открытие базы новым процессом. Она не изменяет ACL и не устанавливает компоненты.
Совместимость с конкретными Windows 10/11 и ограничениями ADR-0001 проверяется отдельно.
