...
## Утверждённые типы отчётов

После утверждения DEV-документации v0.5 раздел 9.8 является источником enum:
`DAILY_MOVEMENT`, `HEAD_SITE`, `SUBSIDIARY`. Для `metric_code` и
`operation_type` полный перечень пока не утверждён, поэтому они сохраняют
проверку формата внешнего uppercase-кода без выдуманных alias.
